    ...
    data2.forEach( record => {
        ...
        test(`test calendar conversions for RD date ${rd}`, () => {
            ...
            expect(d.convertTo(CalendarType.MayanLongCount))
                .toEqual(numericArray(record, ["MayanLongCountBaktun","MayanLongCountKatun","MayanLongCountTun","MayanLongCountUinal","MayanLongCountKin"]))
            expect(d.convertTo(CalendarType.MayanHaab))
                .toEqual(numericArray(record, ["MayanHaabMonth","MayanHaabDay"]))
            expect(d.convertTo(CalendarType.MayanTzolkin))
                .toEqual(numericArray(record, ["MayanTzolkinNumber","MayanTzolkinName"]))
        })
    })
})
